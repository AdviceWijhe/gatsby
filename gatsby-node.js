...
exports.createPages = async gatsbyUtilities => {
  ...
  await createVacatureArchive({ posts, gatsbyUtilities })
...
async function createVacatureArchive({ posts, gatsbyUtilities }) {
  const graphqlResult = await gatsbyUtilities.graphql(/* GraphQL */ `
    {
      wp {
        readingSettings {
    ...
    postsChunkedIntoArchivePages.map(async (_posts, index) => {
      ...
      const getPagePath = page => {
        if (page > 0 && page <= totalPages) {
          ...
          // to be numbered.
          // "/blog/2" for example
          return page === 1 ? `/vacatures/` : `/vacatures/${page}`
        }
      ...
      await gatsbyUtilities.actions.createPage({
        ...
        // use the blog post archive template as the page component
        component: path.resolve(`./src/templates/vacature-archive.js`),
